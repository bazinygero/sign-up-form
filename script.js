const signUpForm = (function signUpValidation() {
    ...
    password.addEventListener('input', () => {
        const minEightcharCheck = document.querySelector('.pass-min-eight');
        const minOneNumCheck = document.querySelector('.pass-min-num');
        const minOneCharCheck = document.querySelector('.pass-min-one')
    ...
    password.addEventListener('focusout', () => {
        ...
        if (/\d/.test(password.value) && password.value.trim().length >= 8 && /[a-zA-Z]/.test(password.value)) {
            password.classList.remove('invalid-input'); 
        } else {
    ...
    createAccountBtn.addEventListener('click', () => {
        if (email.checkValidity() === true && password.value.trim() !== '' && confirmPassword.value === password.value && (/\d/.test(password.value) && password.value.trim().length >= 8 && /[a-zA-Z]/.test(password.value))) {signInForm.submit()}
    })
